...
| 253 | `253-live-world-architecture-convergence` | Convert the live playable Overworld from the legacy 16×64×16 `Chunk` slab to the verified dimension-aware `VerticalWorldAccess`/`ChunkColumn`/`ChunkSection` block-state store; wire `OVERWORLD_DIMENSION_TYPE` from `Game`; migrate generation/streaming/meshing/lighting/simulation/persistence consumers; remove the legacy slab and block-state overlay as independent production authorities; preserve Change-251 furnace and existing saves via deterministic compatibility migration. |
...
| 255 | `255-high-performance-voxel-engine` | Owner-authorized post-253 scalability campaign: validated worker meshing/worldgen, transferable typed payloads, bounded mesh-ready/GPU upload scheduling, intelligent streaming, deterministic far-terrain LOD, dynamic resolution, and reproducible performance certification without gameplay/save/visual-layer regressions. |
| 256 | `256-production-readiness-hardening` | Owner-authorized post-archive hardening (goal 2026-08-31): YAGNI-driven dead-code pruning, magic-number and duplicate-helper consolidation, presentation/type-safety and void-noise hardening, error-handling completion, and measured hot-path/bundle optimization with full behavioral preservation and production-readiness certification. |
| 257 | `257-void-world-startup-recovery` | Owner-authorized live-playability repair (2026-08-31): prevent legacy/unsupported/partial persisted worlds from entering an empty-air/free-fall state; make spawn/readiness baseline-aware; add non-destructive in-product recovery; close real-IndexedDB startup fault coverage; and recertify the full browser gate and repository state. |
| 258 | `258-real-world-runtime-performance-fps-recovery` | Owner-authorized user-visible performance repair: reproduce low FPS in a headed GPU-backed browser, measure whole-frame/subsystem costs, remove main-thread generation/meshing/lighting/upload stalls, activate validated worker paths, enforce adaptive frame budgets, and certify default desktop gameplay with real frame-time/FPS gates without hiding regressions behind headless or quality reductions. |
| 259 | `259-enchanting-panel-ui` | Owner-authorized (2026-09-11) in-game enchanting panel UI over the verified headless enchanting seam (118–120 / 219): place/use an enchanting table opens the panel; item + lapis-like payment + generated offers shown; reselect offers; apply with XP/lapis costs updating inventory/XP with correct persistence; session-guard behavior preserved with browser E2E coverage closing certification debt R-3; close/walk-away/focus/dispose lifecycle safe; original assets only. |
| 260 | `260-live-brewing-stand-production-integration` | Owner-authorized (2026-09-11) live brewing-stand wiring over the verified headless seam (122–123 / 219): place a brewing stand, right-click to open the panel, insert bottle/ingredient/blaze-powder fuel through 106 menu transactions, brew deterministically on the fixed tick in simulating chunks with live progress UI, persist across unload/reload via block-entity persistence, break drops contents safely; close/walk-away/focus/dispose lifecycle safe; unit + browser E2E journey (place→open→insert→brew→collect→reload→break) closing certification debt R-8 (brewing half); original assets only. |
| 261 | `261-gamerule-settings-ui` | Owner-authorized (2026-09-11) in-game settings/gamerule UI over the verified `GameRuleFramework` (189): view/edit all registered gamerules (boolean/integer/string) with validation (invalid = no-op); world-scoped persistence through the game save path; live-consumer wiring for `mobGriefing` (Wither/destroyable paths) and other 189 keys where a production consumer already exists (no new simulation systems); unit + browser E2E (open→toggle→persist→reload→observable effect); original assets only. |
| 262 | `262-recipe-book-ui` | Owner-authorized (2026-09-11) in-game recipe book UI over the verified headless `RecipeBook` (204): open from the crafting UI (`CraftingPanel` flow); known-recipe list with search/filter via the existing helpers (blank query = all known); select shows the `layoutRecipe` ingredient grid with have/missing accounting and crafts when affordable through the transactional `CraftingSystem` (missing = status-surfaced no-op, inventory never corrupted); world-scoped known-set persistence with degrade-to-defaults; unlock on craft output plus craftable-discovery on open (documented rule, book never permanently empty through normal play); unit + browser E2E (open→search→select→craft→reload-preserves); original assets only. |
| 263 | `263-advancement-panel-ui` | Owner-authorized (2026-09-11) in-game advancements panel UI over the verified headless `AdvancementFramework` (185) + `CoreProgressionAdvancements` (186): openable panel (HUD button, one-container rule) listing the 7 core advancements with title/description and progress/completion state; live trigger wiring into Game play (item-obtain choke points at pickup/craft plus a typed `fireAdvancementTrigger` seam for dimension/boss/kill sources) with toast-on-completion; world-scoped `__advancements__` persistence with degrade-to-defaults and reset/archive passthrough; catalog-aware hardened batch deserializer closing the R-4 advancement-half under-validation debt (duplicate/unknown/inconsistent payloads fail closed); unit + browser E2E (open→see-defs→trigger→UI-reflects→reload-persists); original assets only. |
| 264 | `264-item-xp-entity-persistence-hardening` | Owner-authorized (2026-09-11) item/XP entity persistence hardening: `ItemEntityManager`/`XpOrbManager.deserializeAll` reject duplicate ids and malformed payloads fail-closed (deterministic throw, manager unchanged — R-4 entity-manager half); world-scoped live Game persistence for item entities + XP orbs (`__itementities__` / `__xporbs__` raw metadata records, degrade-to-empty quarantine, reset/archive passthrough) so drops/orbs survive chunk unload/reload and page refresh in simulating/persisted regions; Game boot/hydrate uses the hardened deserializer; unit + browser E2E (drop+orb persist across reload); original assets only.
| 265 | `265-live-creative-mode-integration` | Owner-authorized (2026-09-11) live creative-mode integration over the verified headless `GameModeFramework` (192): world-scoped `__gamemode__` persistence with degrade-to-defaults and reset/archive passthrough; `setGameMode` + text-entry seams; creative no-deplete / instant-break / no-stats-drain / minimal safe flight applied live in the Game; creative inventory/menu UI (browse/search placeable blocks/items, grant stacks to hotbar/inventory, no survival crafting gates); survival default and unaffected; unit + browser E2E (enter creative → pick → place-without-depleting → break-instantly → reload-persists; survival still depletes); original assets only. |
| 267 | `267-live-hardcore-mode-integration` | Owner-authorized (2026-09-12) live hardcore-mode integration over the verified headless `HardcoreFramework` (193) alongside the 265/266 mode wiring: world-scoped `__hardcore__` + `__difficulty__` persistence with degrade-to-defaults and reset/archive passthrough (default off / normal); `setHardcore` + `setDifficultyFromText` seams; hardcore locks the effective difficulty to hard and routes death through `respawnModeAfterDeath` into spectator via the 266 spectator path; hardcore toggle + difficulty select in the gamerule-adjacent settings UI; non-hardcore death/respawn unchanged; unit + browser E2E (enable hardcore → die → spectator-locked + hard + reload-persists; normal world still respawns in survival); original assets only. |
| 268 | `268-ci-immutable-action-pins` | Owner-authorized (2026-09-12) CI supply-chain hardening: pin every `actions/*@v4` step in `.github/workflows/ci.yml` and `seed-visual-goldens.yml` to immutable full commit SHAs with `# v4` version comments (checkout / setup-node / cache / upload-artifact SHAs resolved via the GitHub API at authoring time); close certification debt R-5; no `src/`/`tests/` gameplay or behavior change; CI green required on the exact published SHA. |
| 269 | `269-composed-game-dispose-worker-terminate` | Owner-authorized (2026-09-12) composed-Game dispose/worker-terminate hardening: audit `Game.dispose()` + world/worker teardown (`MeshWorkerClient`, `WorkerPool`, `WorldgenWorkerClient` runtime where composed); idempotent dispose that terminates/joins workers so no post-dispose callbacks mutate state or leak handles; no pending Game timers/rAF/listeners after dispose (workers terminated or documented equivalent shutdown); unit/integration tests proving double-dispose safety + worker terminate (mock or real Worker per repo patterns) + a focused e2e/harness where feasible without headed GPU; close certification debt R-6; no gameplay behavior retune; no 258 headed work. |
| 266 | `266-live-adventure-spectator-integration` | Owner-authorized (2026-09-11) live adventure+spectator integration over the verified headless `AdventureModeRules` (194) + `SpectatorFramework` (195) alongside the 265 creative wiring: adventure break/place gated live by the held stack's CanDestroy/CanPlaceOn declarations (new stack component types + `resolveBlockPermissionSet` over block tags; no declared set ⇒ cannot break/place); spectator noclip/no-gravity/no-collision flight via a `PlayerPhysics` hook, no interaction/item-use/container-open via `PlayerInteraction` + Game chokes, no targeting/damage via `isAttackable` (hostile + wither paths; direct damage already gated); survival/creative unchanged; mode switching via the text seam + a HUD mode select (chip toggle unchanged); world-scoped `__gamemode__` persistence already covers all four modes; unit + browser E2E (adventure blocked/permitted, spectator no-interact, survival unaffected, reload persists); original assets only. |
| 270 | `270-leaf-apple-loot-probability` | Owner-authorized (2026-09-12) leaf-apple loot probability over the verified `LootTable` primitives (011): leaves MUST NOT always drop an apple — about 1/200 (0.5%) chance of 1 apple per leaf break through the existing loot-table path with the injected deterministic RNG hook; shears/silk-touch leaf drops (where already special-cased) keep block-drop semantics without forcing an apple; sapling/stick behavior unchanged; unit tests/fixtures updated from the guaranteed-apple assumption with fixed-rng proof tests; closes certification debt R-2; no economy redesign; original assets only. |
| 272 | `272-lighting-clock-dt-sync` | Owner-authorized (2026-09-12) lighting clock/sun dt sync in `src/rendering/Lighting.ts update()`: the same effective (clamped to `CONFIG.maxDeltaTime`, frozen-aware) dt drives both `worldSeconds` and sun rotation (`applyAxisAngle`); frozen (245) advances neither; hitch-sized-dt unit proofs that clock phase and sun angle stay consistent; closes certification debt R-9; no gameplay/systems retune; no golden churn expected (behaviorally invisible at normal frame pacing). |
| 273 | `273-chunksection-isempty-air-check` | Owner-authorized (2026-09-12, final campaign of the current chain) ChunkSection isEmpty air-check closing certification debt R-8 (isEmpty half): `isEmpty()` is true iff the section is entirely air (single-palette whose only entry is `airId`); mono-palette non-air MUST be false; `nonAirCount()` short-circuit and the mesher empty fast-path (`src/world/ChunkMesher.ts`) keep skipping true air sections and MUST NOT skip solid mono-block sections; unit tests (all-air true, all-stone false, mixed false, fill air/stone, serialize round-trip); no golden churn expected (live worldgen builds multi-entry palettes; only the deserialize path changes); no gameplay/systems retune; no 258 headed work. |
| 271 | `271-statistics-panel-ui` | Owner-authorized (2026-09-12) in-game statistics panel UI over the verified headless `StatisticsFramework` (187): world-scoped `__statistics__` persistence with degrade-to-defaults and reset/archive passthrough; live counter increments from real gameplay events (walk distance, blocks broken, mob kills, deaths, damage taken, jumps, time played via play_tick on the fixed tick); openable panel (hotkey + HUD/pause entry, one-container rule) rendering `statisticsSnapshot` values with readable labels and live updates while open; unit + browser E2E (play actions bump stats → panel shows them → reload persists); no key-set redesign; no multiplayer per-player stores; original assets only. |
| 274 | `274-live-sleep-bed-integration` | Owner-authorized (2026-09-13, 274-277 campaign) live sleep/bed integration over the verified headless `SleepFramework` (198): a placeable bed block + bed item (new registry ids, original procedural assets only); bed use interaction path running `enterBed`/`leaveBed` with occupied rejection and night/storm `canSleep` gating (daytime refusal); successful enter sets the world-scoped spawn point, leave keeps it; world-scoped `__sleep__` persistence (versioned, degrade-to-defaults on corrupt/absent, reset delete, archive passthrough); respawn uses the `SleepFramework` spawn point when set (compatible with `respawnPlayer`; hardcore permanent-death routing still wins); minimal toast + HUD sleep indicator (DOM/CSS/text, original assets only); unit tests for the wiring/persistence seams + browser E2E (place/use bed → sleeping/spawn set → leave → reload persists spawn → respawn lands at bed); no multiplayer majority rules, no phantoms, no unrelated refactors, no 258 headed work. |
...
| 279 | `279-shield-live-wiring` | Live shield wiring over verified headless ShieldBlocking (144): shield item + durable offhand equip/swap, held-use raise state, directional blocking for real hostile/wither damage paths, shield durability/break and axe-disable cooldown, HUD state feedback, inventory persistence reuse, unit + browser E2E; no new mob AI or 258 headed work. |
| 280 | `280-death-respawn-ui` | Add an additive live death/respawn presentation over the existing SurvivalSystem and Hardcore/Sleep respawn rules: propagate the existing damage reason into a fail-safe cause label, show a dismissible death card with normal respawn or hardcore-spectator outcome, preserve one-container/input lifecycle, and prove unit/browser regression without a new save namespace or 258 headed work. |
| 281 | `281-workstation-ui` | Add a live player-placed smoker over the verified furnace state/menu/persistence seams: stable block/item identity, twice-fast deterministic processing, shared station-labelled UI, save/reload, lifecycle, and break/drop proof; no villager POI/raid work and no 258 headed work. |
| 282 | `282-live-raid-feedback` | Add bounded live raid feedback over the verified RaidStateMachine: Game-owned ephemeral raid state, fixed-tick progression, an accessible wave/remaining/terminal HUD bar, deterministic test seams for start/clear/replay, and lifecycle-safe hide/reset behavior; no raider entity spawning, settlement detection, persistence namespace, or 258 headed work. |

| 283 | `283-live-raid-persistence` | Add a world-scoped `__raid__` persistence seam behind the verified RaidStateMachine: `RaidPersistence` codec over `serializeRaid`/`deserializeRaid`, `WorldMetadataRepository.putRaidData`/`getRaidData`, GamePersistence bulk-load/save/reset, Game boot hydration + autosave/dispose/pagehide save points, optional `WorldArchive.raidData` with fail-closed import validation and `raidDataImported`, unit + browser E2E reload/reset/corrupt/archive proofs; no raider entity spawning, settlement detection, 282 HUD redesign, or 258 headed work. |
...
| 287 | `287-live-village-detection` | Real spatial settlement/village detection backing the 285 `setVillageQuery` seam: pure Chebyshev bed-scan on resident columns (≥1 `BlockId.Bed` within R=12 / Y=±5), deterministic center + containsPlayer, Game live default query with rate-limited cache, pause/dispose/reload safety, unit + browser E2E; no raider AI, outposts, HOTV, villager spawn, village worldgen, new persistence, or 258 headed work. |
| 288 | `288-raider-combat-behavior` | Make spawned raid-wave entities fight using existing HostileTargetAI / MeleeCombat / ProjectileCore / MobHealthTracker seams: target player or home to raid center; pillager ranged, vindicator/ravager melee, witch ranged fallback; deaths through exactly-once recordRaiderDeath; player death/timeout → DEFEAT; pause/dispose/reload safety; unit + browser E2E; no new AI framework, villagers, HOTV, patrols/outposts, persistence namespace, or 258 headed work. |
| 289 | `289-enchantment-persistence-reload-integrity` | Find/fix root cause of enchantments reading null after pagehide+reload: DirtySaveQueue concurrent-drain integrity + inventory component codec proofs + apply-time player-state save + e2e await real flush signal; before/after repeat-each rates; no storage redesign, no new namespace, no enchanting UI, no 258 headed work. |

| 290 | `290-hero-of-the-village-reward` | Reward raid VICTORY with Hero of the Village via existing status-effect runtime: vanilla-like level/duration from Bad Omen, emerald trading discount (floor 1) shown in trading UI, exactly-once grant, DEFEAT grants nothing, ephemeral effects (no new namespace), unit + browser E2E; no villagers/gifts/patrols/outposts/HUD redesign; 258 stays BLOCKED. |
