...
- [x] 1.1 Establish baseline: record that no golden-image/snapshot infrastructure
...
- [x] 1.2 Add the VITE_E2E-only boot hook so `src/main.ts`/`src/engine/Game.ts`
...
- [x] 1.3 Add the VITE_E2E-only deterministic-state hooks on `window.__voxelGame`:
      ...
      stats) to fixed constants. (Game.testSetCameraPose / testFreezeDayNight /
      testNormalizeHud backed by Lighting.freezeDayNight — analytic sun-direction
      pin solving d=(y+0.18)/1.05 — plus HUD.setFixedText and
      DebugOverlay.setFixedText overrides honored inside their per-frame updates.)

## 2. Pure modules and focused unit tests

- [x] 2.1 Implement `tests/visual/matrix.ts`: `SCREENS` (10), `QUALITY_PROFILES`
      (low/default/high), `RESOLUTIONS` (1280x720/1920x1080), `allCells()` (60),
      `goldenPath(cell)`, and `validateMatrix()`.
- [x] 2.2 Implement `tests/visual/goldenCompare.ts`: `comparePng` supporting
...
- [x] 2.3 Unit tests for `VisualMatrix.test.ts`: screen/family/mode assertions,
      profile values, resolution aspect, 60-cell enumeration, golden-path
      derivation, and `validateMatrix()` valid/invalid cases.
- [x] 2.4 Unit tests for `GoldenCompare.test.ts`: exact-mode equality, pixel-diff
...
- [x] 3.1 Implement `tests/e2e/visual-regression.spec.ts`: per-cell fresh context,
...
- [x] 3.2 Seed the committed goldens under `tests/visual-golden/` by running the
      matrix once with `UPDATE_SNAPSHOTS=1` and reviewing the artifacts.
- [x] 3.3 Verify the full 60-cell matrix passes headlessly against the committed
      goldens (no `UPDATE_SNAPSHOTS`); confirm the matrix is green.
- [x] 3.4 Add failure reporting: per-cell report rows, `test-results/visual/…`
...
- [x] 4.1 Edge/failure coverage: missing golden fails (verify) / writes (update);
...
- [x] 4.2 Confirm the shipped build's `window.__voxelGame`/boot hook gating is
      unaffected when `VITE_E2E` is absent (normal release build has no seam).
- [x] 4.3 Update `tasks.md` checkboxes, `verification.md` with real evidence, and
      the durable state files (`PROGRAM_STATE.json`, `PROGRAM_STATE.md`) when this
      change reaches VERIFIED.
- [x] 4.4 Run the final baseline gate (`npm run typecheck`, `npm run lint`,
