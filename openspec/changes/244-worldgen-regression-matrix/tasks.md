...
- [ ] 1.1 Confirm entry gate: previous change VERIFIED; baseline `npm run typecheck`, `npm run
      lint`, `npm test`, `npm run build`, `npm run test:e2e` green; record baseline unit/E2E
      counts.
- [ ] 1.2 Confirm the produced-world probe surface against the actual code: `TerrainGenerator`
...
- [ ] 2.1 Add `src/worldgen/WorldgenRegressionMatrix.ts`: `MatrixFixtureKind`, `MatrixFixture`,
...
- [ ] 2.2 Implement `verifyWorldgenMatrix` (per-kind actual computation, deterministic
      pass/fail, never throws on mismatch, probe errors surfaced as failed entries with `error`).
- [ ] 2.3 Implement `worldgenMatrixHash` (FNV-1a over canonical `version|key|kind|seed|x|y|z|actual`
...
- [ ] 2.4 Implement `createDefaultWorldgenMatrix` (`v1` catalog, 24–40 fixtures) with the pinned
      values, matrix hash, and registry fingerprint generated once by an authoring script from the
      verified implementation and embedded verbatim.

## 3. Focused unit and edge/failure tests

- [ ] 3.1 Validation tests: every kind valid; empty key/version, unknown kind,
...
- [ ] 3.2 Per-kind `verifyWorldgenMatrix` computation tests against a `TerrainGenerator`-backed
      probe, including a `structurePresent` path using the exact generator/context `TerrainGenerator`
      uses.
- [ ] 3.3 Tests that the full `v1` catalog passes (every fixture `pass: true`), the matrix hash
...
- [ ] 3.4 Seed/coordinate boundary tests: seed 0, seed near 2^31-1, negative and large positive
      `x`/`z`, chunk-boundary `x`/`z ≡ 0 mod 16`, `y` 0 and `y` just below surface.
- [ ] 3.5 Mismatch reporting without throwing (tampered fixture → `pass: false` with actual),
      and probe-error surfacing (a throwing probe → failed entry with `error`, verification
      continues).
- [ ] 3.6 Registry-fingerprint sensitivity: a block-id remap, an altered template block set, and
      an altered placement `spacing` each change the fingerprint; unsupported matrix version
      throws.
- [ ] 3.7 Coverage assertions from the fixtures spec: all four biome ids + origin plains guard,
...
- [ ] 4.1 Confirm the new module does not modify `GoldenSeed.ts` (102) and is additive; run the
      full unit suite to confirm the 102 fixtures and all prior suites stay green.
- [ ] 4.2 Run the baseline gate (`npm run typecheck`, `npm run lint`, `npm test`, `npm run
