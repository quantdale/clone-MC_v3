...
- [x] T3. Register stable smoker block/item ids, placement/drop references,
...
- [x] T4. Add `createSmokerContext` with exact `ceil(base / 2)` duration,
  fail-closed unknown recipes, delegated fuel/result/XP, and focused even/odd/
  boundary tests.
- [x] T5. Add the `smoker` block-entity adapter over the validated FurnaceState
  payload with type-key rejection, lossless timers/XP round-trip, and focused
  unit coverage.

## C. Live host and UI/Game integration

- [x] T6. Extend `LiveBlockEntityHost` for smoker placement, atomic menu writes,
...
- [x] T7. Extend the shared furnace panel and Game station session so Furnace
...
- [x] T8. Wire PlayerInteraction and Game place/use/break/drop paths, smoker
...
- [x] T9. Add a real browser journey covering place → open → label → insert →
...
- [x] T10. Verify archive/persistence expectations: smoker uses only the
...
- [x] T11. Run focused tests and the required regression/gates: typecheck, lint,
...
- [x] T12. Reconcile every artifact and mark C281 exact/281 VERIFIED only at
