...
- [ ] **2.1** Create `src/simulation/MeleeCombat.ts`: `attackCooldownProgress`,
      `cooldownDamageMultiplier`, `computeAttackDamage`, `computeKnockback`,
      `InvulnerabilityTracker`, `resolveMeleeAttack` per design.md.

- [ ] **3.1** Write `tests/unit/MeleeCombat.test.ts`: cooldown-progress bounds/monotonicity;
      ...
      formulas and registers exactly one hit).
