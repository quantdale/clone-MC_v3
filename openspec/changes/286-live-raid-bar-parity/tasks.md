...
## B. Activation gate (completed — 282–285 VERIFIED/published)

- [x] T4. On a session where 282 is VERIFIED (and advancement is allowed),
...
- [x] T5. Confirm baseline: typecheck/lint/unit/build/E2E green at activation
  head; Change 258 still BLOCKED; wither boss-bar regression boundary intact.

## C. Pure projection and isolation

- [x] T6. Implement the pure `projectRaidBar` (or approved equivalent) with
...
- [x] T7. Prove isolation: projection and DOM hooks MUST NOT import or mutate
...
- [x] T8. Wire the distinct raid bar element (additive CSS/DOM) to the
...
- [x] T9. Present village name only from the documented context/fallback and
...
- [x] T10. Edge/failure tests: non-finite counters, empty/whitespace/non-string
...
- [x] T11. Regression: existing `boss-bar.spec.ts`, 282 raid-feedback tests,
...
- [x] T12. Run focused unit + browser suites for projection, a11y, isolation,
  and lifecycle; record exact commands in `verification.md`.
- [x] T13. Full baseline gates: `npm run typecheck`, `npm run lint`,
...
- [x] T14. Reconcile artifacts against every MUST/SHALL; set C286 exact and
