...
- [x] T1. Add the sequential 280 row/override, activate `PROGRAM_STATE` from
  published 279, keep 258 BLOCKED and 259–279 VERIFIED, and reserve 281.
- [x] T2. Complete the SPEC_AUTHORING_PROTOCOL package with the death-cause,
...
- [x] T3. Add the pure death-cause/outcome mapper with closed unknown fallback;
  extend the optional `SurvivalEvent` reason without changing snapshots.
- [x] T4. Add `DeathRespawnPanel` with stable IDs, safe text rendering,
  idempotent dismiss, and focused unit coverage.

## C. Live shell and wiring

- [x] T5. Add the hidden death-screen DOM/CSS card and wire Game to show it
  after the existing respawn/hardcore transition with outcome/cause state.
- [x] T6. Preserve one-container, pointer-lock, reload-hidden, and no-new-
  persistence behavior; add the read/dismiss seams and unit integration proof.

## D. Verification and publication

- [x] T7. Add `tests/e2e/death-respawn.spec.ts` for normal cause/card/dismiss
  and hardcore spectator outcome, while retaining existing 267/274 behavior.
- [x] T8. Run the 259–279 regression and review no unrelated behavior changed.
- [x] T9. Run typecheck, lint, full unit, build, full E2E, file-audit, and
