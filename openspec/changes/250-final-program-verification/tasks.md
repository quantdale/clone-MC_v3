...
- [ ] 1.1 Capture the actual program state at the entry commit: read `openspec/PROGRAM_STATE.json`
...
- [ ] 1.2 Confirm preconditions: change 249 is `VERIFIED` and advancement is allowed;
...
- [ ] 1.3 Run the entry baseline gate (`npm run typecheck`, `npm run lint`, `npm test`,
...
- [ ] 2.1 Author `openspec/evidence/checklist/final-verification-checklist.md`: one row per planned
...
- [ ] 2.2 Classify every not-`VERIFIED` change: record an explicit product decision and rationale
      for each intentional deferral, or mark the row `UNCLASSIFIED`. No `UNCLASSIFIED` row may
      remain for the checklist to pass. (FPC-2)
- [ ] 2.3 Author `openspec/evidence/changes/<NNN>.md` for every change 001–250: `VERIFIED` records
...
- [ ] 2.4 Author `openspec/evidence/README.md` manifest with purpose, catalog (001–250), provenance
...
- [ ] 3.1 Execute the final parity audit pass: re-derive each `PARITY_MATRIX.md` row category from
      ...
      FPA-3)
- [ ] 3.2 Author `openspec/evidence/parity/final-parity-audit.md` recording the audit method, the
      ...
      for `READY`.
- [ ] 3.3 Run the final baseline regression gate (`npm run typecheck`, `npm run lint`, `npm test`,
      `npm run build`, `npm run test:e2e`) and the release performance gate (247), recording every
      command/budget result. (FRS-1, FRS-2)
- [ ] 3.4 Author `openspec/evidence/suites/final-regression-suite.md` with run date/head, overall
      `PASS`/`FAIL`, per-command and per-budget results, and any documented exceptions. (FRS-3)

## 4. Release-readiness decision + final gate

- [ ] 4.1 Evaluate the nine readiness criteria (RC-1..RC-9) against the checklist, archive, audit,
      ...
      RR-2, RR-3)
- [ ] 4.2 Update `openspec/PROGRAM_STATE.json` to mark the program `COMPLETE` (status `COMPLETE`,
      ...
      program. (FPC-3)
- [ ] 4.3 Fill `openspec/changes/250-final-program-verification/verification.md` with the complete
...
- [ ] 4.4 Run the final gate (`npm run typecheck`, `npm run lint`, `npm test`, `npm run build`,
