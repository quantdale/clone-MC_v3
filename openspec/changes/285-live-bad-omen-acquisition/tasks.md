...
- [ ] T3. Add failing/characterization unit tests for `BadOmenRules`
  (clamp/grant/clear/trigger matrix, reason precedence, invalid centers).
- [ ] T4. Implement `src/simulation/BadOmenRules.ts` with total pure helpers
  matching the design API; keep it free of Game, DOM, entity, and persistence
  imports.
- [ ] T5. Wire Game-owned ephemeral `badOmen`, `grantBadOmen` / `clearBadOmen` /
...
- [ ] T6. Add focused unit integration coverage: grant→trigger→raid active +
...
- [ ] T10. Reconcile artifacts to implementation, mark C285 exact/VERIFIED only
