...
- [x] **2.1** Add `serializeChunk(cx, cz)` and `deserializeChunk(cx, cz, entities)` to
      `src/simulation/EntityManager.ts`, per design.md's data model and validate-then-mutate
      atomicity contract.

- [x] **3.1** Extend `tests/unit/EntityManager.test.ts` with `serializeChunk`/`deserializeChunk`
