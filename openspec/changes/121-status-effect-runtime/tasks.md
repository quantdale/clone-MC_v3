...
already exist (`63412f2`, `db5b0e8`). The stale PLANNED/unchecked ledger is reconciled below;
this records existing evidence rather than new implementation work.

## 1. Manager core + serialization

- [x] **1.1** Create `src/data/StatusEffectManager.ts` exporting `EffectAttributeHook`
      ...
      overrides).
- [x] **1.2** Implement `get(typeId)`, `getAll()` (deterministic order), `clear()`,
      `getAttribute(id)`, and `attributes()`.
- [x] **1.3** Implement `serialize()` (map active instances through
...
- [x] **2.1** Implement `add(typeId, duration?, amplifier?)`: resolve the type (strict;
...
- [x] **2.2** Unit test stacking: equal amplifier refreshes to the longer duration and
...
- [x] **3.1** Implement hook apply/remove: on `add`, after storing, add a `Modifier`
      ...
      amplifier.
- [x] **3.2** Unit test: adding `speed` raises `movement_speed.value` by the multiplier;
...
- [x] **4.1** Implement `tick(dt)`: ignore non-finite/negative `dt`; decrement every
...
- [x] **4.2** Unit test: a DURATION_BASED effect ticks to expiry and is removed + unhooked;
...
- [x] **5.1** Cover all requirement scenarios in `spec.md`: add/get/remove/clear,
...
- [x] **6.1** Run `npm run typecheck`, `npm run lint`, `npm test`, `npm run build`,
      `npm run test:e2e`; all green.
- [x] **6.2** Fill `verification.md` with real evidence; mark every task group done.
- [x] **6.3** Advance `openspec/PROGRAM_STATE.json` / `.md` to 121 VERIFIED; set
      `nextChange` to `122` (per `CHANGE_SEQUENCE.md`).
- [x] **6.4** Commit (impl + state bump) and push to `origin/main`; verify remote == local.
