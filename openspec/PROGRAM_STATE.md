...
- Next exact action: **Continue Change 253 task 66: eliminate permanent generation of independently authoritative 64-high slabs. The live `Game` path already uses `generateColumn`; verify and document the remaining `generateChunk` fallback as compatibility-only, with canonical copy-through and no slab persistence. Task 41 browser heap/renderer/entity sampling remains blocked by the standalone headless-Chrome `fillRect()` hang; keep task 41 unchecked and do not weaken readiness or budgets.**
