...
- Next exact action: **Continue Change 253 task 67: preserve deterministic seed/world-version semantics for fresh worlds. Audit the live Game/World/TerrainGenerator seed path and world-version policy, then add or validate deterministic fresh-world evidence without changing the canonical adapter boundary. Task 41 browser heap/renderer/entity sampling remains blocked by the standalone headless-Chrome `fillRect()` hang; keep task 41 unchecked and do not weaken readiness or budgets.**
