...
- Next exact action: **Continue Change 253 PROMPT 02->11 (Phase 2 canonical World wiring: migrate World to CanonicalWorldStorage, remove stateOverlay, convert ChunkManager to column residency, eliminate legacy clamps, wire Overworld dimension from Game). PROMPT 00 (activation) and PROMPT 01 (inventory/characterization) published; PROMPT 02 facade introduced (src/world/CanonicalWorldStorage.ts 8 PASS). Two follow-ups opened by the live-boot repair, both recorded in `verification.md`: gate the visual-regression capture on streaming quiescence instead of `#loading` + fixed delay, and re-baseline the stale win32-local goldens against converged 253 terrain under owner authorization. Execute PROMPT 00->11: governance -> inventory -> canonical core -> Overworld generation/streaming -> section rendering/lighting -> gameplay/simulation migration -> persistence/migration -> modularity -> performance -> post-audit -> verification/E2E -> publish+CI. Keep build green; publish coherent checkpoints to origin/main.**
