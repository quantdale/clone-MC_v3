...
- Next exact action: **Continue Change 253 task 64: audit the current `src/worldgen/**` stage graph and document the exact live adapter/composition chosen. Task 23/41 browser heap/renderer/entity sampling remains blocked by the standalone headless-Chrome `fillRect()` hang; keep task 41 unchecked, preserve the `Chunk` compatibility boundary, and do not weaken readiness or budgets.**
