...
import type { ReplayRecording, ReplayTickSeed } from '../../src/simulation/ReplayRecording';
...
function streamSeeds(stream: string, maxTick: number, start: number, drawsPerTick: number): ReplayRecording['tickSeeds'] {
...
function drawOptions(): ReplayRunnerOptions {
  return { makeSystems: (streams: ReplayStreamRegistry) => [new DrawSystem((n) => streams.get(n))] };
}
  ...
  it('reports seed_mismatch at the tick where a tampered seed no longer reproduces', () => {
    ...
    const tampered: ReplayRecording = {
      ...
      tickSeeds: rec.tickSeeds.map((ts, i) =>
        i === 0
          ? { tick: ts.tick, seeds: [{ stream: 'alpha', state: (ts.seeds[0]!.state ^ 0xabcdef) >>> 0 }] }
          : ts,
      ),
    };
...
describe('ReplayVerifier: divergence diagnosis', () => {
  function traceOf(hashes: number[], version = 'v1', divergence = null as ReplayTrace['divergence']): ReplayTrace {
    return {
      version,
  ...
  it('reports the first diverging tick as a hash divergence', () => {
    ...
    expect(cmp.firstDivergence).toEqual({ kind: 'hash', tick: 3, expected: 30, actual: 99 });
  });
  ...
  it('refuses cross-version comparison with version_mismatch', () => {
    ...
    expect(cmp.identical).toBe(false);
    expect(cmp.firstDivergence).toEqual({ kind: 'version_mismatch', expected: 'v1', actual: 'v2' });
  });
});
...
describe('ReplayVerifier: failure and version handling', () => {
  ...
  it('surfaces a mid-replay system failure naming the tick, hashing no later ticks', () => {
    ...
    const trace = runRecording(rec, { makeSystems: (streams) => [new ThrowingSystem(), new DrawSystem((n) => streams.get(n))] });
    const div = trace.divergence!;
    expect(div).not.toBeNull();
  ...
  it('rejects an unsupported recording version', () => {
    ...
    expect(() => runRecording(rec, drawOptions())).toThrow(/unsupported recording version/);
  });
  ...
  it('never calls a failure-expecting trace identical to a successful run', () => {
    ...
    const expectedFailure: ReplayTrace = {
      ...
      divergence: { kind: 'system_failure', tick: 4, error: new Error('expected') },
    };
...
describe('ReplayVerifier: integration with WorldTickProcess', () => {
  it('reproduced hashes match a fresh run and the production driver path', () => {
    ...
    expect(again.ticks.map((t) => t.hash)).toEqual(trace.ticks.map((t) => t.hash));
    ...
    expect(last.hash).toBe(hashState({ tick: maxTick, systems: [{ draws: prodDraws }] }));
  });
});
