/**
 * Typed IndexedDB repository boundary for block-entity records (036). The `IDBFactory` is injectable
 * so the repository is fully unit-testable in Node without a browser global; production code passes
 * the browser adapter (`browserIdbFactory`). The `block-entities` store lives in the same
 * `voxel-world-db` database as the 034/035 stores; the shared `ensureWorldStores` upgrade routine
 * (in `WorldMetadataRepository`) creates all object stores, so opening this repository at the current
 * `WORLD_DB_VERSION` migrates a v2 database forward. The repository stores plain `BlockEntityChunkRecord`
 * data and is deliberately decoupled from any live block-entity framework; the `data` payload is opaque.
 */
import {
  WORLD_DB_NAME,
  WORLD_DB_VERSION,
...
import {
  ...
  type BlockEntityChunkRecord,
  type SerializedBlockEntity,
} from './BlockEntityRecord';
...
/**
 * Typed boundary for persisting and reloading block-entity records per chunk. The factory is
...
export class BlockEntityRepository {
  ...
  private readonly store = WORLD_BLOCK_ENTITY_STORE;
  ...
  private requireDb(): IdbDatabaseLike {
    if (!this.db) {
      throw new Error('BlockEntityRepository: open() must be called before use');
    }
    return this.db;
  }
  ...
  /** Validate and persist the block entities for a chunk under its composite key. */
  async putChunkEntities(
    ...
    const valid = validateBlockEntityChunkRecord({ worldId, chunkX, chunkZ, entities });
    const key = chunkKey(worldId, chunkX, chunkZ);
    const record: BlockEntityChunkRecord = { ...valid, key };
    await promisifyRequest(this.storeHandle().put(record));
  }

  /** Return the block entities for `(worldId, chunkX, chunkZ)`, or `null` if absent. */
  async getChunkEntities(
    worldId: string,
    chunkX: number,
    chunkZ: number,
  ): Promise<SerializedBlockEntity[] | null> {
    const key = chunkKey(worldId, chunkX, chunkZ);
    const result = await promisifyRequest(this.storeHandleReadOnly().get(key));
    if (result === undefined || result === null) return null;
    return (result as BlockEntityChunkRecord).entities;
  }

  /** Return all chunk records belonging to `worldId` (empty array when none). */
  async listChunks(worldId: string): Promise<BlockEntityChunkRecord[]> {
    const result = await promisifyRequest(this.storeHandleReadOnly().getAll());
    const all = (result as BlockEntityChunkRecord[]) ?? [];
    return all.filter((r) => r.worldId === worldId);
  }

  /** Delete the block-entity record for `(worldId, chunkX, chunkZ)`. */
...
/** Re-export the transaction surface for consumers/tests that mock the factory directly. */
