...
export function ensureWorldStores(db: IdbDatabaseLike): void {
  ...
  if (!db.objectStoreNames.contains(WORLD_CHUNK_EDIT_STORE)) {
    db.createObjectStore(WORLD_CHUNK_EDIT_STORE, { keyPath: 'key' });
  }
}
...
export class WorldMetadataRepository {
  ...
  /** Raw put for the recipe-book payload bypassing WorldMetadata validation (262; gamerule precedent). */
  async putRecipeBookData(worldId: string, payload: unknown): Promise<void> {
    const raw = {
      worldId: `__recipebook__:${worldId}`,
      payload,
      updatedAt: Date.now(),
    };
    const tx = this.requireDb().transaction(this.store, 'readwrite');
  ...
  /** Raw put for the item-entity payload bypassing WorldMetadata validation (264; advancement precedent). */
  async putItemEntityData(worldId: string, payload: unknown): Promise<void> {
    const raw = {
      worldId: `__itementities__:${worldId}`,
      payload,
      updatedAt: Date.now(),
    };
    const tx = this.requireDb().transaction(this.store, 'readwrite');
  ...
  /** Raw put for the sleep payload bypassing WorldMetadata validation (274; statistics precedent). */
  async putSleepData(worldId: string, payload: unknown): Promise<void> {
    const raw = {
      worldId: `__sleep__:${worldId}`,
      payload,
      updatedAt: Date.now(),
    };
    const tx = this.requireDb().transaction(this.store, 'readwrite');
  ...
  /** Raw get for the item-entity payload (264). Returns null when absent. */
  async getItemEntityData(worldId: string): Promise<unknown | null> {
    const tx = this.requireDb().transaction(this.store, 'readonly');
    const result = await promisifyRequest(tx.objectStore(this.store).get(`__itementities__:${worldId}`)) as { payload?: unknown } | undefined;
    if (!result || !('payload' in (result as Record<string, unknown>))) return null;
    return (result as { payload: unknown }).payload ?? null;
  }

  /** Raw get for the XP-orb payload (264). Returns null when absent. */
  async getXpOrbData(worldId: string): Promise<unknown | null> {
    const tx = this.requireDb().transaction(this.store, 'readonly');
    const result = await promisifyRequest(tx.objectStore(this.store).get(`__xporbs__:${worldId}`)) as { payload?: unknown } | undefined;
    if (!result || !('payload' in (result as Record<string, unknown>))) return null;
    return (result as { payload: unknown }).payload ?? null;
  }
  ...
  /** Raw get for the sleep payload (274). Returns null when absent. */
  async getSleepData(worldId: string): Promise<unknown | null> {
    const tx = this.requireDb().transaction(this.store, 'readonly');
    const result = await promisifyRequest(tx.objectStore(this.store).get(`__sleep__:${worldId}`)) as { payload?: unknown } | undefined;
    if (!result || !('payload' in (result as Record<string, unknown>))) return null;
    return (result as { payload: unknown }).payload ?? null;
  }
